...
1. **Beale-Kato-Majda (BKM) Logarithmic Grönwall Regularity Criterion**:
   Machine-checked proof that finite-time $L^\infty$ vorticity integrability strictly prevents finite-time blow-up in higher Sobolev spaces $H^s(\mathbb{T}^3)$ via double-exponential Grönwall majorization.
...
3. **Full 4-Stage Caffarelli-Kohn-Nirenberg (CKN 1982) Partial Regularity**:
   - **Stage 1 (`NS3D_CKN_Stage1_ParabolicCylinders.lean`)**: Parabolic spacetime metric scaling and volume scaling $\operatorname{Vol}(Q_r) = \frac{4\pi}{3} r^5$.
   ...
   - **Stage 3 (`NS3D_CKN_Stage3_EpsilonRegularity.lean`)**: Dimensionless scale-invariant enstrophy functionals $E(r)$ and $\varepsilon$-regularity propagation.
   - **Stage 4 (`NS3D_CKN_Stage4_ZeroMeasure.lean`)**: Vitali covering energy control, proof that parabolic 1D Hausdorff measure vanishes $\mathcal{H}^1(\operatorname{Sing}(\boldsymbol{u})) = 0$, and strict mathematical exclusion of 2D surface singularities (vortex sheets / pancakes).
...
| `NS3D_BKM_Criterion.lean` | `vorticity_strain_energy_absorption` | `prod - 2*nu*D ≤ 2 * norm_Linf(omega) * E` | `[propext, Classical.choice, Quot.sound]` | **100% Proved** |
